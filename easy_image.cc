...
img::Color::Color(std::vector<int> vec) {
	this->red = vec[0];
	this->green = vec[1];
	this->blue = vec[2];
}
...
void img::EasyImage::clear_gradient(img::Color color1, img::Color color2) {
...
void img::EasyImage::draw_line_rainbow(unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1) {
	assert(x0 < this->width && y0 < this->height);
	assert(x1 < this->width && y1 < this->height);
	if (x0 == x1)
	{
		...
		for (unsigned int i = std::min(y0, y1); i <= std::max(y0, y1); i++)
		{
			Color color = Color(256*(double)x0/this->width, 256*(double)i/this->height, 256*((double)i/this->height + (double)x0/this->width));
			(*this)(x0, i) = color;
		}
	}
	else if (y0 == y1)
	{
		...
		for (unsigned int i = std::min(x0, x1); i <= std::max(x0, x1); i++)
		{
			Color color = Color(256*(double)i/this->width, 256*(double)y0/this->height, 256*((double)y0/this->height + (double)i/this->width));
			(*this)(i, y0) = color;
		}
	}
	else
